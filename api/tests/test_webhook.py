...
@pytest.mark.asyncio
async def test_webhook_idempotent_same_event_id(admin_client):
    body = _payload()
    headers = {
        "X-Signature": sign(WEBHOOK_SECRET, body),
        "Content-Type": "application/json",
    ...
    first = await admin_client.post(
        "/api/webhooks/tickets", content=body, headers=headers
    )
    ...
    listing = await admin_client.get("/api/tickets")
